...
that gives a great overview of R Markdown  
\- Yihui’s [Rmd book](https://bookdown.org/yihui/rmarkdown/) for lots
more on R Markdown.
...
Notice the syntax for:
  ...
  - <span style="color: red;">change text color with</span>`<span
...
Make the following changes to your RMarkdown document: \* Add some
italic text \* Make a numbered list \* Add some subheaders
...
Chunk output can be customised with **options**, arguments supplied to
...
executed and what results are inserted in the finished report:
  ...
  - `error = TRUE` causes the render to continue even if code returns an
    ...
    default, `error = FALSE` causes knitting to fail if there is a
    single error in the document.
...
by posting it to the general channel in Slack.
...
GitHub can render it nicely on its website. To knit an RMarkdown filt to
...
This set up is a one-time thing\! You will only have to do this once per
computer. We’ll walk through this together. In a browser, go to
github.com and to your profile page as a reminder.
...
We will be using the `use_git_config()` function from the `usethis`
package. We first need to install this package. Type the following in a
new script:
...
If you see `Error in use_git_config() : could not find function
"use_git_config"` please run `library("usethis")`

### Ensure that Git/GitHub/RStudio are communicating

We are going to go through a few steps to ensure the Git/GitHub are
communicating with RStudio

#### RStudio: New Project

Click on New Project. There are a few different ways; you could also go
to File \> New Project…, or click the little green + with the R box in
the top left. also in the File menu).

<br>

<img src="../img/new_project_1.png" width="80%" />
...
<img src="../img/new_project_2.png" width="80%" />
...
<img src="../img/new_project_3.png" width="80%" />
...
<img src="../img/new_project_4.png" width="80%" />
...
2.  Troubleshooting, starting with [HappyGitWithR’s troubleshooting
    chapter](http://happygitwithr.com/troubleshooting.html)
...
1.  Potentially set up a RStudio Cloud account:
<https://rstudio.cloud/>
...
All troubleshooting starts with reading Happy Git With R’s [RStudio,
Git, GitHub Hell](http://happygitwithr.com/troubleshooting.html)
...
\-\> If yes, then the [Git you
downloaded](https://git-scm.com/downloads) isn’t installed, please
redownload if necessary, and follow instructions to install.
...
[here](https://classroom.github.com/a/SA7QIA7g)
